...
function generateNoiseTexture(regl: REGL.Regl, size: number) {
  const l = size * size * 2;
  ...
  for (let i = 0; i < l; i++) {
    ...
    const r0 = Math.cos(r);
    const r1 = Math.sin(r);
    array[i * 2 + 0] = Math.round(0.5 * (1.0 + r0) * 255);
    array[i * 2 + 1] = Math.round(0.5 * (1.0 + r1) * 255);
  }
  return regl.texture({
...
export function createRenderer(regl: REGL.Regl) {
  const pgWidth = 256;
